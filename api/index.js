...
var loadEnvVariables = () => {
  ...
  if (missingVariables.length > 0) {
    ...
    if (
      missingVariables.includes("DATABASE_URL") ||
      missingVariables.includes("BETTER_AUTH_SECRET")
    ) {
...
var sendEmail = async ({ subject, templateData, templateName, to }) => {
  try {
...
var getCategories = async (query) => {
  const categoryQuery = new QueryBuilder_default(query)
...
var getAllUsers = async (query) => {
  const userQuery = new QueryBuilder_default(query)
    .search(["name", "email"])
    .filter()
    .sort()
    .paginate();
...
var getAllIdeas = async (query) => {
  const ideaQuery = new QueryBuilder_default(query)
...
var deleteIdea = async (ideaId, userId, userRole) => {
  ...
  if (userRole === Role.MEMBER) {
    if (isExistIdea.status !== IdeaStatus.DRAFT) {
      throw new AppError_default(
        status9.FORBIDDEN,
        "You can only delete DRAFT ideas",
      );
    }
  }
...
var getIdeaById2 = catchAsync_default(async (req, res) => {
  ...
  const result = await IdeaService.getIdeaById(req.params.id, userRole, userId);
  sendResponse_default(res, {
    statusCode: status10.OK,
    success: true,
...
var updateIdea2 = catchAsync_default(async (req, res) => {
  ...
  const userId = req.user?.userId;
  const payload = req.body;
  const result = await IdeaService.updateIdea(id, payload, userId, userRole);
  sendResponse_default(res, {
    statusCode: status10.OK,
    success: true,
...
var createPaymentIntent = async (userId, ideaId) => {
  ...
  if (!idea.isPaid)
    throw new AppError_default(status13.BAD_REQUEST, "Free idea");
...
var confirmWebhook = async (signature, payload) => {
  ...
  if (
    ...
    if (userId && ideaId) {
      console.log(
        `\u2705 Webhook: Updating payment to COMPLETED for Idea ${ideaId}, User ${userId}`,
      );
      ...
      console.log(
        `\u26A0\uFE0F Webhook: Received ${event.type} but missing metadata. Falling back to Intent ID lookup.`,
      );
      await prisma.payment.update({
...
var createPayment = catchAsync_default(async (req, res) => {
  const userId = req.user.userId;
  const { ideaId } = req.params;
  const data = await PaymentService.createPaymentIntent(userId, ideaId);
...
var webhook = async (req, res) => {
  try {
    ...
    const result = await PaymentService.confirmWebhook(signature, req.body);
