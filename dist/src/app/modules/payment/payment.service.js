...
const createPayment = async (userId, ideaId) => {
    ...
    if (existingPayment && existingPayment.status === "COMPLETED") {
        throw new AppError(status.BAD_REQUEST, "You have already paid for this idea");
    }
...
const checkPaymentStatus = async (userId, ideaId) => {
...
const handleWebhook = async (signature, payload) => {
    let event;
    try {
        event = stripe.webhooks.constructEvent(payload, signature, envVars.STRIPE.WEBHOOK_SECRET);
    }
    catch (error) {
